...
#ifdef CONFIG_LGE_DISPLAY_LV9_JDI_NT35596
...
#ifdef CONFIG_LGE_DISPLAY_LV9_JDI_NT35596
	calling = atomic_read(&ts->state.incoming_call);
	if (calling && !flag_knock_on_ctrl_during_panel_on) {
		TOUCH_I("%s - Skip sleep control when Wake up DDIC.\n", __func__);
		return ret;
	}
#endif
	touch_msleep(120);
...
int synaptics_tci_report_enable(struct device *dev, bool enable)
{
	...
	calling = atomic_read(&ts->state.incoming_call);
	if (calling && !flag_knock_on_ctrl_during_panel_on) {
		TOUCH_I("%s - Skip LPWG Command when Wake up DDIC.\n", __func__);
		return ret;
	}
#endif
...
static ssize_t show_glove_test(struct device *dev, char *buf)
{
	...
	if (synaptics_is_product(d, "PLG636", 6)) {
		...
		switch (mfts_mode) {
			case 0:
				if (ts->factory_boot)
					fname = "/data/touch/touch_glove_test.txt";
				else
					fname = "/sdcard/touch_glove_test.txt";
				break;
			case 1:
				fname = "/data/touch/touch_glove_test_mfts.txt";
				break;
			default:
